...
import Foundation
import UIKit
import CWStatusBarNotification  // 第三方库关于消息通知
...
enum CommonPhoneDeviceModel : String {
    ...
    case iPhone5;case iPhone5c;
    case iPhone5s ;case iPhone5se
    ...
    case iPadMini ;case iPadMini2;
    case iPadMini3;case iPadMini4
    case iPadPro;case AppleTV;
    case Simulator;case  processor
}
    ...
    class func getRequestDataBlock(_ urlString:String, _ param:String, block Block: @escaping (_ json:NSDictionary) -> Void) {
        ...
        let dataTask = session.dataTask(with: request as URLRequest) { (data, response, error) -> Void in
            if (error != nil) {
                ...
                DispatchQueue.main.async {  // 放在主线程当中
                    //返回数据
    ...
    class func getJsonData(urlString:String, param:String){
        ...
        let dataTask = session.dataTask(with: request as URLRequest) { (data, response, error) -> Void in
            if (error != nil) {
                ...
                DispatchQueue.main.async {  // 放在主线程当中
                    //返回数据
                    CommonFunction.getRequestDataClosuresCallBack!(json as! NSDictionary)
                }
            }
        }
        // 启动任务
...
final class CommonFunction {
   ...
   // 第三方 CWStatusBarNotification
    ...
    static let IOS10 = (UIDevice.current.systemVersion as NSString).doubleValue >= 10.0
...
//            UIApplication.shared.openURL(url!)
    ...
    /// 颜色
    static func RGBA(_ r:CGFloat, g:CGFloat, b:CGFloat) -> UIColor {
...
//    // MARK:HUD (菊花等待加载进度...) 需要第三方库MBProgressHUD
//    /**
